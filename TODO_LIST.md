...
| Reduce `applyTriage` cognitive complexity (36, limit 35)       | ⬜ `TODO` | Low    | Low    | `pipeline/pipeline_triage.go`                     |
...
| Rename unused `s` receivers in `saboteurProvider` test mock    | ⬜ `TODO` | Low    | Low    | `pipeline/fix_provider_test.go` (revive)          |
