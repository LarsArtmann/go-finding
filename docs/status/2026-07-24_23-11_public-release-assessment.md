...
| #  | Task |
|----|------|
| 1  | Make `GOEXPERIMENT=jsonv2` prominent in README Installation section (move to top, add `go env -w` one-liner) |
| 2  | Remove `GOPRIVATE` warning from README (lines 46-47) |
| 3  | Add GitHub repo description: `gh repo edit --description "Unified data model and pipeline for Go static analysis tools"` |
| 4  | Add GitHub topics: `gh repo edit --add-topic go,static-analysis,sarif,lsp,code-quality,golang,static-analysis-tools` |
| 5  | ~~Verify `go-output` is public~~ ✅ `DONE` | Confirmed by user |
| 6  | ~~Verify `gogenfilter` is public~~ ✅ `DONE` | Confirmed by user |
...
| 11 | Verify `LICENSE` file is standard MIT text |

### Phase 2: Community Health (before announcing)

| #  | Task |
|----|------|
| 12 | Create `SECURITY.md` (vulnerability reporting policy) |
| 13 | Create `CODE_OF_CONDUCT.md` (Contributor Covenant v2.1) |
| 14 | Create `.github/ISSUE_TEMPLATE/bug_report.yml` |
| 15 | Create `.github/ISSUE_TEMPLATE/feature_request.yml` |
| 16 | Create `.github/PULL_REQUEST_TEMPLATE.md` |
| 17 | Add support policy section to README ("MIT license, best-effort support, no SLA") |
| 18 | Decide on 91 internal docs: move `docs/status/` + `docs/planning/` + `docs/reviews/` to `docs/internal/` OR add `.gitignore` entries OR keep |
| 19 | Add "Internal docs" note to README if keeping them |
...
| 22 | Verify `examples/` directory compiles and is referenced in README |
| 23 | Add `FUNDING.yml` (optional — GitHub Sponsors) |

### Phase 3: Launch

| #  | Task |
|----|------|
| 24 | Tag v1.4.0 as the first public version anchor |
...
| 40 | Create a logo/visual identity for the project |
| 41 | Create a project website (Astro + Starlight pattern) |
| 42 | Add badge for Go Reference (pkg.go.dev) once verified rendering |
| 43 | Monitor first week of issues/PRs — respond within 48h |
| 44 | Write a "v2.0 vision" roadmap entry (what would justify a major bump) |
...
| 48 | Run brutal-self-review skill before going public |
| 49 | Verify all 4 module tags resolve on `proxy.golang.org` after visibility flip |
| 50 | Create a "Migration guide for consumers" (removing GOPRIVATE, adding GOEXPERIMENT) |
...
### Q2: Do you want the 91 internal docs (status reports, planning docs, self-critiques, architecture reviews) to remain public, be moved to `docs/internal/`, or be gitignored?

This is a judgment call about how much internal process you want to expose. Arguments for each:
- **Keep:** Full transparency, shows rigorous engineering process
- **Move to `docs/internal/`:** Reduces top-level noise while preserving history
- **Gitignore:** Cleanest public face, but loses the narrative

I cannot decide this for you — it depends on your personal brand and how you want the project perceived.
