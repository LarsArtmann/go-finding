...
7. Run remaining CI structural scripts locally once (`test-naming`, `docs-freshness`, `docs-api-check`, `json-deterministic`, `replace-audit`) to convert "near-certainly fine" into verified.
8. Full-workspace `-race` pass (all modules, all packages) — cheap confidence beyond the 2 packages raced this session.

**Docs / changelog:**
9. CHANGELOG entries: root (sarif_import refactor), pipeline (exitResult/newApplyReport/nearestBy refactors), CLI (installFlightRecorder + resolve() + error wording change).
10. Check `docs/guides/flight-recorder.md` error-path examples against new wording; fix if stale.
11. Fix AGENTS.md date nit (2026-09-22 → 2026-09-22/23 session).
12. Harvest this section (f) into TODO_LIST.md per docs-health HARVEST (awaiting user go-ahead).
