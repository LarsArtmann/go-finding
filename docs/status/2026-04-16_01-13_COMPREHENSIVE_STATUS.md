...
9. **Missed PROPOSAL.md alignment** — Moving `diagnostic.go` out actually *aligns* with "SDK depends on nothing" principle. This strong argument wasn't used.
...
The PROPOSAL.md says *"Converters live in each tool, not in the SDK. Tools depend on the SDK; the SDK depends on nothing"* — which is library-first. But the CLI, GoReleaser, and examples suggest CLI-first. This tension needs resolving before committing to the module split.
