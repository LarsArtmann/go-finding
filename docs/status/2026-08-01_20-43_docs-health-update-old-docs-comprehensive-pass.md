...
| --- | ------------------------------------------------------------------- | ------ | ------ | ----------------------------------------------------------------------------------------------------- |
| 1   | Tag/release the unreleased FlightRecorder work as v1.5.0 or v1.4.2  | High   | Low    | CHANGELOG `[Unreleased]` is populated; feature complete and tested; no tag exists                     |
| 2   | Commit benchmark baseline (remove `/benchmarks/` from `.gitignore`) | High   | Low    | TODO_LIST MEDIUM — CI regression check has never worked                                               |
| 3   | Unit tests for FlightRecorder extracted helpers                     | High   | Medium | `2026-08-01_19-40` self-critique flagged: `recordStageBoundary`, `asyncSnapshot` lack dedicated tests |
| 4   | Full FEATURES.md vs code audit (every section, every method list)   | High   | High   | Recurring gap flagged in every docs-health session since v1.3.0                                       |

### 🟡 MEDIUM Priority

| #   | Task                                                                                     | Impact | Effort | Evidence                                                                                  |
| --- | ---------------------------------------------------------------------------------------- | ------ | ------ | ----------------------------------------------------------------------------------------- |
| 5   | CONTRIBUTING.md project tree refresh                                                     | Medium | Low    | ~10 files missing; flagged since v1.3.0                                                   |
| 6   | API_STABILITY.md symbol tables for v1.3.0+ APIs                                          | Medium | Medium | Missing Template, BuildOrDefault, FilePos, SeverityFromLevel, ApplySimpleFixes, etc.      |
| 7   | MIGRATION_v1.3.md consumer migration guide                                               | Medium | Medium | Never created; 12 convenience APIs undocumented for migration                             |
| 8   | Harvest remaining STILL OPEN items from historical reports                               | Medium | Medium | ~15 items identified but not routed (`.envrc`, goexperiment guard, per-module lint, etc.) |
| 9   | Annotate 4 remaining HTML review reports (code-quality, dedup, modularize, architecture) | Medium | Low    | Self-documenting but no formal resolution footers                                         |
| 10  | Update D2 architecture diagram for FlightRecorder                                        | Medium | Low    | `current-architecture.d2` doesn't mention FlightRecorder in Pipeline box                  |
| 11  | README Pipeline Features table — add Flight Recorder row                                 | Medium | Low    | I forgot this (see §d.4)                                                                  |
| 12  | Read and harvest `docs/status/2026-08-01_19-40_flight-recorder-self-critique.md`         | Medium | Low    | Found but not acted on; has actionable open items                                         |
| 13  | `docs/guides/fix-engine.md` — document ApplySimpleFixes                                  | Medium | Low    | Flagged since v1.3.0                                                                      |
| 14  | FlightRecorder: add `Snapshot` error context and `Enabled()` race documentation          | Medium | Low    | Self-critique flagged these                                                               |
...
| 18  | go-linter-sdk: wire `IsEnabledByDefault` into `Registry.Run`                             | Medium | Medium | Sibling repo; interface exists but registry ignores it; false contract                    |
...
| #   | Task                                                            | Impact | Effort  | Evidence                                                                                   |
| --- | --------------------------------------------------------------- | ------ | ------- | ------------------------------------------------------------------------------------------ |
| 21  | Add `.envrc`/direnv for GOEXPERIMENT=jsonv2                     | Low    | Low     | Flagged in 5+ historical reports; never done                                               |
| 22  | Add `//go:build goexperiment.jsonv2` source guard on json.go    | Low    | Low     | Build-time error instead of runtime "build constraints" error                              |
| 23  | Per-module golangci-lint configs                                | Low    | Medium  | Flagged since modularization; workspace-level lint suffices but loses per-module precision |
| 24  | go-arch-lint module boundary enforcement CI                     | Low    | Medium  | Flagged in modularization reports; never added                                             |
| 25  | `go.work sync` idempotency CI check                             | Low    | Medium  | Flagged in modularization reports                                                          |
| 26  | Replace directive audit CI check                                | Low    | Low     | Flagged in modularization reports                                                          |
| 27  | Version drift detection CI check (beyond version-check.sh)      | Low    | Low     | Current check only covers tag vs version.go                                                |
| 28  | CI check rejecting `_extra_test.go`/`_bugfix_test.go` filenames | Low    | Low     | Convention only; no automated enforcement                                                  |
| 29  | CHANGELOG enforcement CI (fail PR if CHANGELOG not updated)     | Low    | Low     | Flagged in 2+ reports                                                                      |
| 30  | Docs-freshness CI check                                         | Low    | Medium  | Flagged in 2+ reports                                                                      |
| 31  | Per-module CHANGELOG entries                                    | Low    | Medium  | Flagged in modularization reports                                                          |
| 32  | Benchmark: multi-module vs monolith overhead                    | Low    | Medium  | Flagged in modularization reports                                                          |
| 33  | Benchmark: SARIF/LSP/FilePath round-trip                        | Low    | Medium  | Thin benchmark coverage for serialization paths                                            |
| 34  | SARIF schema validation test                                    | Low    | High    | TODO_LIST BLOCKED — requires vendoring 7K+ line SARIF 2.1.0 JSON schema                    |
| 35  | Consumer compatibility test                                     | Low    | High    | TODO_LIST BLOCKED — repo is private; consumers need GOPRIVATE                              |
| 36  | `CODEOWNERS` file for `.github/workflows/`                      | Low    | Low     | Flagged in PR sweep reports; never created                                                 |
...
| 40  | `Finding.ValidateAll()` batch helper                            | Low    | Low     | Flagged in post-M12-M22 report                                                             |
| 41  | Runtime TOCTOU symlink swap test                                | Low    | Medium  | Flagged in hardening self-review; unit tests exist but no runtime swap test                |
| 42  | `resolveSafePath` as exported API                               | Low    | Low     | Currently package-private; flagged as potential public boundary                            |
...
| 45  | `//go:build goexperiment.jsonv2` on all 9 json/v2 files         | Low    | Low     | Compile-time guard instead of runtime error                                                |
| 46  | Markdown link-checker CI job                                    | Low    | Low     | No automated link checking; manual grep only                                               |
| 47  | Fuzz target for `resolveSafePath`                               | Low    | Medium  | Flagged in hardening self-review                                                           |
| 48  | `Finding.Equal` property-based test                             | Low    | Low     | Tag order-insensitive equality needs property test                                         |
| 49  | `Range.Contains`/`Overlaps` property-based test                 | Low    | Low     | Flagged in correctness sweep                                                               |
