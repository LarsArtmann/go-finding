...
2. **The full paste arrived** with one NEW forensic detail: the `errors.go` violation showed *my `//nolint:erraudit` reason text inside the flagged code*. That is only possible under `--no-suppress`.
...
*Type-model candidates (API-affecting, needs a driver):*
