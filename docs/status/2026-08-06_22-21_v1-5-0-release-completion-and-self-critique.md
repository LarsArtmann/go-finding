...
| 5   | **Proved all 6 JSON determinism tests are real regression guards** | Temporarily removed `json.Deterministic(true)` from all 6 call sites in `json.go`. Ran `go test -run TestDeterminism_Finding|TestDeterminism_Report`. ALL 6 FAILED. Previous session claimed they were "confidence tests" — they are not. |
