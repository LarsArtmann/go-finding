...
4. **Make "flaky gate" verification a written step, not a virtue.** Every FAIL path this session was verified by *actually breaking something in a disposable context* (clone probe, stray worktree, sed-injected stamps). The dead-gate lesson (2026-09-08) already mandates this; institutionalize it as a checklist row in release-procedure step 0 so it survives sessions.
...
Ranked by impact; Effort: S <30min, M 30min–2hr, L >2hr. *(Brainstorm, not commitment — HARVEST should route: items 1–12 → TODO_LIST, the rest → ROADMAP unless promoted.)*
