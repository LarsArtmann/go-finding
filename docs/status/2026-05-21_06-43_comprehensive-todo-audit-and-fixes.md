...
AGENTS.md (global) mandates: *"Never use Makefile — use `flake.nix` for all build/task automation in LarsArtmann projects"* and *"justfile is deprecated"*. But this project has no `flake.nix` at all — it uses a `justfile` as its primary build tool. The TODO list has 5 phases of Nix migration as open items.
