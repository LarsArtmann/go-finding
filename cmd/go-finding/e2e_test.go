...
func TestRun_E2E_FilterGenerated(t *testing.T) {
	...
	cmd := exec.CommandContext(context.Background(), bin, "-dir="+tmpDir, "-format=json", "-filter-generated")
	out, err := cmd.CombinedOutput()
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(string(out)).To(ContainSubstring(`"findings"`))

	cmdTypes := exec.CommandContext(context.Background(), bin, "-dir="+tmpDir, "-format=text", "-filter-generated", "-filter-generated-types=sqlc,mockgen")
