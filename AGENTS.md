...
- **TEMPORARY: lychee excludes `pkg\.go\.dev` in ci.yml** — the pkg.go.dev page 404s until indexing of the freshly-public module completes; the exclude keeps the link job green meanwhile. REMOVE the exclude once https://pkg.go.dev/github.com/larsartmann/go-finding renders (dead-gate risk if forgotten).
