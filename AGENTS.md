...
- **All production JSON/SARIF marshal calls pass `json.Deterministic(true)`** — `encoding/json/v2` serializes Go map keys in unspecified order by default (unlike v1's alphabetical sort). Without `json.Deterministic(true)`, every call to `MarshalJSON`, `PrettyJSON`, `PrettyJSONFiltered`, `WriteJSON`, `ToSARIFWithOpts`, `WriteSARIFWithOpts`, `LineJSON`, or `Finding.WriteJSON` produces map keys (SARIF `properties` bag, `Summary.bySeverity`/`byCategory`/`byFixStrategy`, `Finding.Metadata`) in random order on each run. Any new marshal call in production code MUST pass `json.Deterministic(true)` or consumers that snapshot/diff output will be flaky.
...
- **CLI config-file flight recorder fallback** — `cmd/go-finding/config.go` has its own `flightRecorderFileConfig` (3 fields: `Enabled`, `OutputDir`, `SlowStageThreshold`) — intentionally simpler than the pipeline's 5-field version (no `MinAge`/`MaxBytes`). The CLI uses config-file flight recorder settings when `-trace` is not set on the command line. This means the CLI config struct is NOT the same as `pipeline.FlightRecorderFileConfig`; they evolved independently.
- **CI scripts guard multi-module architecture** — Four scripts in `scripts/` protect the 4-module structure: `replace-audit.sh` (verifies all replace directives point to `../`), `version-drift.sh` (cross-checks that all go.mod files reference the same core version), `test-naming.sh` (enforces one-test-file-per-production-file + no `_extra`/`_bugfix`/`coverage` suffixes), `go-work-sync.sh` (verifies go.work entries match directory structure). These are wired into `.github/workflows/ci.yml` as separate jobs.
